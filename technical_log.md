...
> Internal development log. Captures every major design decision, the
> alternatives considered, test failures encountered and how they were
> resolved. Personal reflection on GenAI use is deliberately **out of
> scope** for this file — it is a raw engineering diary the author can
> mine when writing their own reflection for the video.
...
* GenAI critical reflection: 15 % (owner: user, not me — I supply the
  raw engineering diary they will mine).
...
so a marker can sketch the development timeline from `git log
--oneline` alone.
