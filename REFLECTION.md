...
> Author: Ali Abdelatif - COMP3011 Web Services and Web Data, CW2.
> Tools declared: Claude (Anthropic).
> Mode of use: AI as a pair-programmer through most of the build.

---

This reflection looks honestly at what I learned from working
with AI on this build - not whether AI is "useful" in the
abstract. The honest answer is "both more and less than I
expected," and the interesting cases are where those two collide.

## 0. Alternative AI tools considered

Before settling on a single tool I weighed a few options:

* GitHub Copilot - built for in-IDE line-level tab completion rather
  than multi-file architectural conversations. Useful for fast code
  synthesis but weaker on design dialogue.
* ChatGPT (OpenAI) and Gemini (Google) - comparable conversational
  assistants. Both can handle the technical content here.
* Claude (Anthropic) - what I used. I chose it because its longer
  effective context window let the brief, the technical log, and the
  full source tree stay in one conversation without me re-priming
  each turn.

Using a single tool throughout (rather than rotating between several)
kept the development pattern consistent and made the bugs I describe
in sections 1 and 2 easier to attribute back to the specific
assistant.
